...
import { U as getPublicPrograms, E as getAssetUrl } from "./router-78ZVeuz1.mjs";
import "../_libs/firebase.mjs";
import "../_libs/firebase__messaging.mjs";
import { a9 as Search, ae as SlidersHorizontal, as as X, ah as Star, x as Clock, aq as Users } from "../_libs/lucide-react.mjs";
...
function CoursesPage() {
  ...
  const specialties = reactExports.useMemo(() => {
  ...
  const filtered = reactExports.useMemo(() => {
    let r = courses.filter(
      (c) => (!q || c.title.toLowerCase().includes(q.toLowerCase())) && (spec.length === 0 || spec.some((s) => s.trim().toLowerCase() === (c.specialty || c.category || "").trim().toLowerCase())) && (dur.length === 0 || dur.includes(c.duration || "6 Months"))
